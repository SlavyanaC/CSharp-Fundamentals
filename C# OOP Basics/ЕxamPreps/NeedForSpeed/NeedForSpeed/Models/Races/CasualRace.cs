    ...
    public override string ToString()
    {
        ...
        else
        {
            ...
            foreach (var car in orderedCars)
            {
                if (counter == 3)
                {
                    builder.AppendLine($"{counter++}. {car.Value.Brand} {car.Value.Model} {car.Key}PP - ${this.ThirdPrize}");
                }
                if (counter == 2)
                {
                    builder.AppendLine($"{counter++}. {car.Value.Brand} {car.Value.Model} {car.Key}PP - ${this.SecondPrize}");
                }
                if (counter == 1)
                {
                    builder.AppendLine($"{counter++}. {car.Value.Brand} {car.Value.Model} {car.Key}PP - ${this.FirstPrize}");
                }
            }
        }
